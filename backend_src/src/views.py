...
@csrf_exempt
def insert_questions_and_answers(request, exam_id): # sau khi update phải cập nhật lại thời gian thay đổi
    if request.method == "POST":
...
def query_questions_and_answers_by_examid(request, exam_id):
    if request.method == "GET":
        ...
        return JsonResponse({"q_and_a": q_and_a_serializer.data, "duration": exam_collections.data['duration']}, safe=False)
@csrf_exempt
def insert_test_result(request, exam_id):
    if request.method == "POST":
        ...
        return JsonResponse({"id":test_result_pk, "data":test_result_ser.data}, safe=False)
    if request.method == "PATCH":
        test_result = TEST_RESULT.objects.get(
            id=exam_id
        )
        data = json.loads(request.body)
...
def insert_test_result_specific(request, exam_id):
    if request.method == "POST":
        ...
        for data in dataList:
            ...
            questions_and_answers.save()
        questions_and_answers_seri = test_result_specific_serializer(questions_and_answers)
