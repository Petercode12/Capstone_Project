...
class HighlightApp extends Component {
  ...
  tooltipRenderer(lettersNode, range, rangeIndex, onMouseOverHighlightedWord, color) {
      ...
      <Tooltip
        ...
        overlay={
          <div>
            ...
            <span className="highlight-icon highlight-color red" 
            onClick={()=> {
              console.log("Set color red"); 
              this.setState({color: "#FF3333"});
              }}/>
            <span className="fas fa-trash highlight-icon highlight-remove trashmovetop" onClick={this.resetHightlight.bind(this, range)}/>
          </div>
        }
        defaultVisible={true}
        animation="zoom"
...
function mapStateToProps(state) {
  return {
    ranges: state.app.get("ranges", new Map({color: "#000"})),
  };
}
